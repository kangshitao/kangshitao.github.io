...
将$\mathbf{h}^l \in \mathbb{R}^{d\times m}$用$\mathbf{u}\in \mathbb{R}^{d\times n}$表示(?没看懂)，然后加入可学习的位置编码$\mathbf{p}\in \mathbb{R}^{d\times d}$，则satellite节点表示变为$\mathbf{u}^p = \mathbf{u}+\mathbf{p}$。
